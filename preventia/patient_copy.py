ASSISTANT_DISCLOSURE = (
    "Le escribe PreventIA, un asistente virtual del consultorio. "
    "No soy una persona ni un profesional de la salud, y no reemplazo sus controles. "
    "Cada día le voy a preguntar si tomó sus remedios y cómo se ha sentido. "
    "Lo que usted me cuente queda anotado para su equipo de salud."
)

CRISIS_DIVERSION = (
    "Gracias por contarme algo así de difícil. Usted no tiene que pasar por esto sin apoyo. "
    "Estoy avisando ahora mismo a su equipo de salud para que se comuniquen con usted lo antes "
    "posible. Por favor manténgase acompañado y deje su teléfono cerca."
)

SAFE_REDIRECT = (
    "Prefiero no responderle eso, porque no soy médico. "
    "Estoy dejando su mensaje anotado para el equipo del consultorio, "
    "que se va a comunicar con usted."
)

URGENCY_REDIRECT = (
    "Por lo que me cuenta, le pido que se comunique hoy con su consultorio. "
    "Si se siente peor, llame al 131 o vaya al servicio de urgencia más cercano. "
    "Ya avisé a su equipo de salud."
)

TEAM_NOTIFIED = "Ya avisé a su equipo de salud para que se comunique con usted."

FALLBACK_ACKNOWLEDGEMENT = (
    "Gracias por contarme cómo se ha sentido. "
    "Dejé anotado su mensaje para el equipo del consultorio."
)

UNSUPPORTED_MEDIA_REPLY = (
    "Recibí su mensaje, pero por ahora solo puedo leer texto. "
    "¿Me lo puede escribir, por favor?"
)

DELIVERY_FAILED_NOTICE = (
    "No pude enviarle mi respuesta. Voy a intentarlo de nuevo en un momento."
)

CONSOLE_GREETING = "Escriba su respuesta y presione Enter. Para salir, escriba: salir"

CONSOLE_FAREWELL = "Conversación cerrada."
